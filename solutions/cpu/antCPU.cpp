...
public:
	...
	Pheromones(int treeCount, ant bestAnt) {
		...
		{
			set(bestAnt.cutted[x - 1], bestAnt.cutted[x], get(bestAnt.cutted[x - 1], bestAnt.cutted[x]) + 1);
		}
	}
	~Pheromones() {
		for (int i = 0; i < size - 1; i++) {
...
void updatePheromones() {
	...
	for (int ant = 0; ant < ANTS; ant++)
	{
		...
		{
			double newval = pheromones->get(ants[ant]->cutted[a - 1], ants[ant]->cutted[a]) + (ants[ant]->res / (double)best); // + (1 / double(ants[ant]->total))
...
int main(int argc, char** argv)
{
	...
	trees[0].setValues( 0, 0, 0, 0, 0, 0 ); // create fake tree on starting point
	for (int a = 1; a <= treeCount; a++)
	{
		cutted[a] = false;
	}
